    ...
    return `
        <div class="menu-item">
            <div class="menu-item-row">
                <span class="menu-item-name">${esc(i.name)}</span>
                ${i.badge ? `<span class="badge" style="${badgeStyle(i.badge_color)}">${esc(i.badge)}</span>` : ""}
                <span class="leader"></span>
                <span class="menu-item-price">€ ${Number(i.price).toFixed(2)}</span>
            </div>
            ${i.description ? `<p class="menu-item-desc">${esc(i.description)}</p>` : ""}
        </div>`;
}
...
function categoryHTML(cat, items, index) {
    ...
    return `
        <section class="menu-category reveal ${dir}">
            ${cat.image_url ? `
                <div class="cat-photo">
                    <img src="${esc(cat.image_url)}" alt="${esc(cat.name)}">
                </div>` : ""}
            <div class="cat-body">
